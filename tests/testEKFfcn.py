...
for i in range(0,gps_data.shape[0]):
    gps = gps_data.iloc[i]
    ins = ins_data.iloc[i*ins_gps_pub_ratio]

    P_pri, X_pri, residual, predicted_state, R_bias_prior, A_bias_prior = ekf.ekf_step(gps["field.twist.twist.linear.y"],gps["field.twist.twist.linear.x"],
                 ins["field.angular_velocity.z"],ins["field.linear_acceleration.x"],
...
plt.suptitle("Inputs")
axes_in[0].plot(gps_data["field.twist.twist.linear.y"])
axes_in[0].set_xlabel("GNSS linear vel y")
axes_in[1].plot(gps_data["field.twist.twist.linear.x"])
axes_in[1].set_xlabel("GNSS linear vel x")
axes_in[2].plot(ins_data["field.angular_velocity.z"])
axes_in[2].set_xlabel("INS angular vel z")
axes_in[3].plot(ins_data["field.linear_acceleration.x"])
axes_in[3].set_xlabel("INS linear acceleration x")
plt.show()